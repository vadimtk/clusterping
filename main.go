...
func pinger(idx int) {
        ...
        for i < pings {
                ...
                fmt.Printf("node: %d, ping: %d, time: %s\n", idx, i, td.TimeDiff)
                i = i + 1
        }
...
func main() {
        ...
        db1.MustExec("CREATE TABLE __clusterping (id int not null primary key, pingtime DATETIME(6) not null)")
        ...
        for i := 1; i < pings; i++ {
                fmt.Printf("ping: %d\n", i)
                db1.MustExec("INSERT INTO __clusterping (id,pingtime) VALUES (" + strconv.Itoa(i) + ",NOW(6))")
